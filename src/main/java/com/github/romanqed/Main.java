...
public class Main {
    public static void main(String[] args) throws IOException {
        Document document = Jsoup.connect("https://ficbook.net/requests/576328").get();
        parseRawRequestPage(document);
    }
    ...
    public static void parseRequests(Document document) {
        ...
        requests.forEach(Main::parseRequestView);
    }
    ...
    public static void parseReviews(Document document) {
        ...
        reviews.forEach(Main::parseReview);
    }
...
//        Document doc = Jsoup.connect("https://ficbook.net/readfic/8989993").get();
        ...
        pairings.forEach(pairing -> {
            System.out.println("<--->");
            System.out.println("Ссылка: " + pairing.attr("href"));
            System.out.println("Название: " + pairing.text());
            System.out.println("<--->");
        });
        // Размер
...
//        Element article = articles.first();
        ...
        Elements fandoms = fanficHat.select("a");
        fandoms.forEach(fandom -> {
            System.out.printf("Ссылка: %s Название: %s%n", fandom.attr("href"), fandom.text());
        });
        // Пейринги/Персонажи
